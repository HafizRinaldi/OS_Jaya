...
# ============================================================
#  SIDEBAR (LOGO > MENU > FILTER > PENJELASAN > COPYRIGHT)
# ============================================================
...
with st.sidebar:
    ...
    st.markdown(
        ...
        Catatan: **CRITICAL = status CRITICAL pada sistem SISS**.  
...
REPORT_URL_SISS = (
    ...
    "?&requestOnDemand=%7B%22beginTs%22%3A1764781200000%2C%22endTs%22%3A1764867599999%7D"
)
...
def fetch_siss_raw() -> str:
    ...
    for extra in attempts:
        ...
        resp = session.get(REPORT_URL_SISS, headers=headers)
...
def parse_siss_to_df(raw_text: str) -> pd.DataFrame:
    ...
    # Filter hanya NORMAL & CRITICAL dari sistem,
    # lalu tampilkan CRITICAL sebagai CRITICAL di dashboard
    if "Status" in df.columns:
        df = df[df["Status"].isin(["NORMAL", "CRITICAL"])].copy()
        df["Status"] = df["Status"].replace({"CRITICAL": "CRITICAL"})
...
def page_siss(status_filter: str | None):
    st.markdown('<div class="section-title">🛰️ SISS Site Status</div>', unsafe_allow_html=True)
    st.caption("Data Site List dari SISS (status NORMAL & CRITICAL, diambil dari sistem SISS).")
...
">
  ...
  <div style="flex: 1;">
    ...
    <div style="font-size: 13px;">
      Update terakhir: {info_waktu} (WIB). Klik tombol <b>Refresh Data SISS</b> untuk mengambil data terbaru.
    </div>
  </div>
</div>
"""
    ...
    if "df_siss" in st.session_state:
        ...
        if {"latitude", "longitude", "Status"}.issubset(df_filtered.columns):
            ...
            # Icon URL: merah untuk CRITICAL, hijau untuk NORMAL (tanpa icon telpon)
        ...
        # =================================================
        # Lanjutan: Download, tabel & grafik seperti biasa
        # =================================================
